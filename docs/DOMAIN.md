...
**Важно:** сейчас `keymaster.pp.ua` и `www` часто смотрят на **не Vercel** (например `135.181.41.169`) — тогда в Vercel будет **Invalid Configuration**. Удали старые A/CNAME на чужой IP и поставь записи **из Vercel** (Domains → домен → **Learn more** / DNS Records).
...
### Vercel: apex и www без петли редиректов

- Основной сайт: **`keymaster.pp.ua`** → **Connect to Production** (не «Redirect to www»).
- **`www.keymaster.pp.ua`** → **Redirect** на `https://keymaster.pp.ua` (308/301).

Если apex настроен как редирект на www, а в коде наоборот — браузер может крутить редиректы.
