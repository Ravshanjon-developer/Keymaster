...
/** Starter plan allows only 3 pages — map logical names onto 3 physical pages. */
const PHYSICAL_PAGES = ['01–03 Screens', '04–05 System', '06–07 Mobile'];
const PAGE_ALIASES = {
  '01 — Product Map': '01–03 Screens',
  '02 — User Flows': '01–03 Screens',
  '03 — Existing Screens': '01–03 Screens',
  '04 — Design System': '04–05 System',
  '05 — Components': '04–05 System',
  '06 — Improved Screens': '06–07 Mobile',
  '07 — Mobile Screens': '06–07 Mobile',
};
...
/** Figma Plugin API has no createAutoLayout — use Frame + layoutMode. */
function createAutoLayout(dir) {
  const f = figma.createFrame();
  f.layoutMode = dir === 'HORIZONTAL' ? 'HORIZONTAL' : 'VERTICAL';
...
function pageByName(name) {
  const physical = PAGE_ALIASES[name] || name;
  return figma.root.children.find((p) => p.type === 'PAGE' && p.name === physical);
}
...
function card(name, w, children) {
  const f = createAutoLayout('VERTICAL');
  f.name = name;
...
async function ensurePages() {
  ...
  existing[0].name = PHYSICAL_PAGES[0];
  await figma.setCurrentPageAsync(existing[0]);
  for (let i = 1; i < PHYSICAL_PAGES.length; i++) {
    const name = PHYSICAL_PAGES[i];
    const pages = figma.root.children.filter((p) => p.type === 'PAGE');
    if (pages.find((p) => p.name === name)) continue;
...
async function buildProductMap() {
  ...
  const root = createAutoLayout('VERTICAL');
  ...
  const layouts = createAutoLayout('HORIZONTAL');
...
async function buildLayoutChrome(parent) {
  ...
  const board = createAutoLayout('HORIZONTAL');
...
async function buildDesignSystemBoard() {
  ...
  root.appendChild(row);
  const typeRow = createAutoLayout('VERTICAL');
...
async function buildComponents() {
  ...
  function inputVariant(name, border, shadow) {
    ...
    c.layoutSizingVertical = 'HUG';
    const field = createAutoLayout('HORIZONTAL');
  ...
  function lang(name, selected) {
    ...
    c.fills = [TRANSPARENT];
    const ru = createAutoLayout('HORIZONTAL');
    ...
    const tj = createAutoLayout('HORIZONTAL');
...
async function buildPlaceholder() {
  ...
  const frame = createAutoLayout('VERTICAL');
...
async function buildScreenHeaders() {
  for (const pageName of ['03 — Existing Screens', '07 — Mobile Screens']) {
    ...
    const header = createAutoLayout('VERTICAL');
...
async function buildVisualFlows() {
  ...
  const flows = [
    ...
    ['F5 First laptop', [
      ...
      ['desktop-learner-simulator-desktop-menupaste.jpg', 'Desktop paste menu'],
      ...
      ['desktop-guest-lesson-task.jpg', 'Guest task lesson'],
    ]],
    ['F6 Hotkey learn', [
      ['desktop-learner-17-lesson-hotkey.jpg', 'KeyboardTrainer'],
      ['desktop-guest-lesson-hotkey.jpg', 'Guest hotkey gate'],
    ]],
    ['F7 Practice hub', [
      ...
      ['desktop-learner-training-hint3.jpg', 'Training hint 3'],
      ['desktop-learner-training-retry.jpg', 'Training retry'],
      ['desktop-learner-training-explain.jpg', 'Training explain'],
      ['desktop-learner-training-correct.jpg', 'Training correct'],
      ['desktop-learner-training-done-weak.jpg', 'Training done weak'],
    ...
    ['F8 Reinforce', [
      ...
      ['desktop-learner-quiz-finished.jpg', 'Quiz finished'],
      ['desktop-learner-16-exam.jpg', 'Exam setup'],
      ['desktop-learner-exam-capped.jpg', 'Exam capped'],
      ...
      ['desktop-learner-exam-certificate.jpg', 'Exam certificate'],
    ...
    ['F11 Admin', [
      ...
      ['desktop-admin-edit-lesson.jpg', 'Edit lesson'],
...
function al(dir, name) {
  const f = createAutoLayout(dir === 'VERTICAL' ? 'VERTICAL' : 'HORIZONTAL');
  f.name = name || 'Frame';
  return f;
}
  ...
  const groups = [
    ...
    [
      ...
      [
        ...
        ['/quiz', 'Quiz', 'play + done'],
        ['/exam', 'Exam', 'setup | capped | loading | run | feedback | done | empty'],
      ],
    ],
    [
      'ImmersiveSimulator',
...
async function buildUniqueScreens() {
  ...
  const lessonTaskGuest = al('VERTICAL', 'Guest task lesson');
  lessonTaskGuest.itemSpacing = 16;
  const guestTaskCard = al('VERTICAL', 'guest task card');
  guestTaskCard.itemSpacing = 12;
  guestTaskCard.paddingTop = guestTaskCard.paddingBottom = 28;
  guestTaskCard.paddingLeft = guestTaskCard.paddingRight = 28;
  guestTaskCard.cornerRadius = 24;
  guestTaskCard.fills = [solid(WHITE)];
  guestTaskCard.resize(720, 10);
  guestTaskCard.layoutSizingHorizontal = 'FIXED';
  guestTaskCard.layoutSizingVertical = 'HUG';
  guestTaskCard.appendChild(txt('Регистрация для практики', outfit('Bold'), 11, BRAND));
  guestTaskCard.appendChild(txt('Файл и папка', fraunces('Bold'), 28, INK));
  guestTaskCard.appendChild(
    txt(
  ...
  const guestTaskHint = al('VERTICAL', 'guest task prompt');
  guestTaskHint.paddingTop = guestTaskHint.paddingBottom = 14;
  guestTaskHint.paddingLeft = guestTaskHint.paddingRight = 16;
  guestTaskHint.cornerRadius = 16;
  guestTaskHint.fills = [solid({ r: 0.941, g: 0.945, b: 0.953 })];
  guestTaskHint.appendChild(txt('Поймите разницу между файлом и папкой', outfit('SemiBold'), 14, BRAND800, 640));
  guestTaskCard.appendChild(guestTaskHint);
  const guestTaskGate = al('VERTICAL', 'task PracticeRegisterGate');
  guestTaskGate.itemSpacing = 8;
  guestTaskGate.paddingTop = guestTaskGate.paddingBottom = 16;
  guestTaskGate.paddingLeft = guestTaskGate.paddingRight = 16;
  guestTaskGate.cornerRadius = 16;
  guestTaskGate.fills = [solid({ r: 0.941, g: 0.945, b: 0.953 })];
  guestTaskGate.appendChild(txt('ПРАКТИКА', outfit('Bold'), 11, BRAND800));
  guestTaskGate.appendChild(txt('Создайте бесплатный аккаунт', outfit('SemiBold'), 16, INK));
  guestTaskGate.appendChild(
    txt(
      'Тренажёр клавиш, XP и путь обучения доступны после регистрации. Каталог и теория уроков — без аккаунта.',
      outfit('Regular'),
      13,
  ...
  const guestTaskBtns = al('HORIZONTAL', 'guest task ctas');
  guestTaskBtns.itemSpacing = 8;
  guestTaskBtns.appendChild(instPrimary('Регистрация'));
  guestTaskBtns.appendChild(instSecondary('Уже есть аккаунт'));
  guestTaskGate.appendChild(guestTaskBtns);
  guestTaskCard.appendChild(guestTaskGate);
  lessonTaskGuest.appendChild(guestTaskCard);
  lessonTaskGuest.appendChild(txt('← К каталогу', outfit('SemiBold'), 13, BRAND800));
  ...
  const loginVerified = authScreen(
    'Вход',
    'Добро пожаловать в KeyMaster',
    [
      (() => {
        const banner = al('HORIZONTAL', 'verified banner');
        banner.paddingLeft = banner.paddingRight = 12;
        banner.paddingTop = banner.paddingBottom = 8;
        banner.cornerRadius = 12;
        banner.fills = [solid(SUCCESS, 0.1)];
        banner.strokes = [solid(SUCCESS, 0.3)];
        banner.appendChild(txt('Email подтверждён. Можно войти.', outfit('Medium'), 13, { r: 0.082, g: 0.502, b: 0.239 }, 296));
        return banner;
      })(),
      floatOrInst('State=Default', 'Email'),
      floatOrInst('State=Password', 'Пароль', { password: true }),
      instPrimary('Войти'),
    ],
    'Нет аккаунта? Регистрация',
  );
  ...
  const quizFinished = al('VERTICAL', 'Quiz finished card');
  quizFinished.itemSpacing = 10;
  quizFinished.primaryAxisAlignItems = 'CENTER';
  quizFinished.paddingTop = quizFinished.paddingBottom = 40;
  quizFinished.paddingLeft = quizFinished.paddingRight = 32;
  quizFinished.cornerRadius = 24;
  quizFinished.fills = [solid(WHITE)];
  quizFinished.resize(420, 10);
  quizFinished.layoutSizingHorizontal = 'FIXED';
  quizFinished.layoutSizingVertical = 'HUG';
  quizFinished.appendChild(txt('Основы hotkeys — готово', outfit('SemiBold'), 22, INK));
  quizFinished.appendChild(txt('8 из 25', outfit('Bold'), 28, BRAND800));
  quizFinished.appendChild(txt('Заработано: 40 XP', outfit('Regular'), 13, MUTED));
  quizFinished.appendChild(txt('Лучшая серия: 2', outfit('Regular'), 13, MUTED));
  quizFinished.appendChild(instPrimary('Ещё раз'));
  ...
  const examCapped = al('VERTICAL', 'Exam capped');
  examCapped.itemSpacing = 12;
  examCapped.appendChild(txt('ЭКЗАМЕН', outfit('Bold'), 11, BRAND800));
  examCapped.appendChild(txt('Настройка сессии', fraunces('Bold'), 32, INK));
  examCapped.appendChild(
    txt(
      'Выберите курс, объём и лимит времени. По истечении минут экзамен завершится автоматически.',
      outfit('Regular'),
      13,
  ...
  const examCappedCard = al('VERTICAL', 'GlassCard capped');
  examCappedCard.itemSpacing = 16;
  examCappedCard.paddingTop = examCappedCard.paddingBottom = 24;
  examCappedCard.paddingLeft = examCappedCard.paddingRight = 24;
  examCappedCard.cornerRadius = 24;
  examCappedCard.fills = [solid(WHITE)];
  examCappedCard.appendChild(txt('Курс', outfit('SemiBold'), 13, INK));
  examCappedCard.appendChild(field('Курс', 'Первый ноутбук: файлы и папки', INK));
  examCappedCard.appendChild(txt('Вопросов в курсе: 16', outfit('Regular'), 12, MUTED, 600));
  examCappedCard.appendChild(txt('Число вопросов', outfit('SemiBold'), 13, INK));
  const qCappedRow = al('HORIZONTAL', 'questions capped');
  qCappedRow.itemSpacing = 8;
  for (const n of [10, 20, 30, 50]) {
    const on = n === 50;
    const muted = n > 16;
    const chip = al('HORIZONTAL', String(n));
    ...
    chip.minHeight = 44;
    chip.cornerRadius = 12;
    chip.opacity = muted && !on ? 0.45 : 1;
    chip.fills = [solid(on ? BRAND : WHITE)];
    chip.strokes = [solid(on ? BRAND : INK, on ? 1 : 0.12)];
    chip.appendChild(txt(String(n), outfit('SemiBold'), 13, on ? WHITE : INK));
    qCappedRow.appendChild(chip);
  }
  examCappedCard.appendChild(qCappedRow);
  examCappedCard.appendChild(txt('Лимит времени', outfit('SemiBold'), 13, INK));
  const mCappedRow = al('HORIZONTAL', 'minutes capped');
  mCappedRow.itemSpacing = 8;
  for (const n of [5, 10, 15, 20]) {
    const chip = al('HORIZONTAL', String(n));
    chip.paddingLeft = chip.paddingRight = 14;
    chip.paddingTop = chip.paddingBottom = 10;
    ...
    chip.strokes = [solid(n === 10 ? BRAND : INK, n === 10 ? 1 : 0.12)];
    chip.appendChild(txt(n + ' мин', outfit('SemiBold'), 13, n === 10 ? WHITE : INK));
    mCappedRow.appendChild(chip);
  }
  examCappedCard.appendChild(mCappedRow);
  const cappedSummary = al('VERTICAL', 'Ваша сессия capped');
  cappedSummary.itemSpacing = 12;
  cappedSummary.paddingTop = cappedSummary.paddingBottom = 16;
  cappedSummary.paddingLeft = cappedSummary.paddingRight = 16;
  cappedSummary.cornerRadius = 16;
  cappedSummary.fills = [solid({ r: 0.976, g: 0.98, b: 0.984 })];
  cappedSummary.appendChild(txt('ВАША СЕССИЯ', outfit('Bold'), 11, MUTED));
  const cappedMetrics = al('HORIZONTAL', 'session metrics capped');
  cappedMetrics.itemSpacing = 8;
  for (const [k, v] of [
    ['Число вопросов', '16'],
    ['Лимит времени', '10 мин'],
    ['Темп', '~38 сек.'],
    ...
    m.appendChild(txt(k, outfit('Bold'), 10, MUTED));
    m.appendChild(txt(v, outfit('SemiBold'), 16, INK));
    cappedMetrics.appendChild(m);
  }
  cappedSummary.appendChild(cappedMetrics);
  cappedSummary.appendChild(pill('Первый ноутбук: файлы и папки', { r: 0.859, g: 0.914, b: 0.996 }, BRAND800));
  cappedSummary.appendChild(
    txt('В курсе меньше вопросов, чем выбрано. Будет задано: 16', outfit('Regular'), 12, { r: 0.706, g: 0.325, b: 0.035 }, 600),
  );
  cappedSummary.appendChild(
    txt('Сбалансированный темп: успеете вспомнить сочетание и нажать его.', outfit('Regular'), 12, MUTED, 600),
  );
  examCappedCard.appendChild(cappedSummary);
  examCappedCard.appendChild(instPrimary('Начать экзамен'));
  examCapped.appendChild(examCappedCard);
  ...
  const trainDoneWeak = al('VERTICAL', 'Training done weak card');
  trainDoneWeak.itemSpacing = 10;
  trainDoneWeak.primaryAxisAlignItems = 'CENTER';
  trainDoneWeak.paddingTop = trainDoneWeak.paddingBottom = 40;
  trainDoneWeak.paddingLeft = trainDoneWeak.paddingRight = 32;
  trainDoneWeak.cornerRadius = 24;
  trainDoneWeak.fills = [solid(WHITE)];
  trainDoneWeak.resize(420, 10);
  trainDoneWeak.layoutSizingHorizontal = 'FIXED';
  trainDoneWeak.layoutSizingVertical = 'HUG';
  trainDoneWeak.appendChild(txt('Тренировка завершена', outfit('SemiBold'), 22, INK));
  trainDoneWeak.appendChild(txt('Выполнено 2 из 2 заданий', outfit('Regular'), 13, MUTED));
  trainDoneWeak.appendChild(txt('Лучшая серия: 2', outfit('Regular'), 12, MUTED));
  trainDoneWeak.appendChild(txt('Стоит повторить:', outfit('SemiBold'), 14, INK));
  const weakRow = al('HORIZONTAL', 'weak lesson');
  weakRow.primaryAxisAlignItems = 'SPACE_BETWEEN';
  weakRow.counterAxisAlignItems = 'CENTER';
  weakRow.paddingTop = weakRow.paddingBottom = 8;
  weakRow.paddingLeft = weakRow.paddingRight = 12;
  weakRow.cornerRadius = 12;
  weakRow.fills = [solid({ r: 0.976, g: 0.98, b: 0.984 })];
  weakRow.resize(356, 10);
  weakRow.layoutSizingHorizontal = 'FIXED';
  weakRow.layoutSizingVertical = 'HUG';
  weakRow.appendChild(txt('Вырезать', outfit('Regular'), 13, INK));
  const weakKeys = al('HORIZONTAL', 'weak KeyCombo');
  weakKeys.itemSpacing = 6;
  weakKeys.counterAxisAlignItems = 'CENTER';
  for (const k of ['Ctrl', 'X']) {
    const cap = instKeyCap(k);
    if (cap) {
      weakKeys.appendChild(cap);
      continue;
    }
    const box = al('HORIZONTAL', k);
    box.paddingLeft = box.paddingRight = 8;
    box.paddingTop = box.paddingBottom = 4;
    box.cornerRadius = 8;
    box.fills = [solid(WHITE)];
    box.strokes = [solid(INK)];
    box.appendChild(txt(k, outfit('SemiBold'), 12, INK));
    weakKeys.appendChild(box);
  }
  weakRow.appendChild(weakKeys);
  trainDoneWeak.appendChild(weakRow);
  trainDoneWeak.appendChild(instPrimary('Пройти ещё раз'));
  ...
  function deskWin(title, pathLabel, countLabel, empty, trashFile, zipFile, folderRow) {
    ...
    if (empty) main.appendChild(txt('Эта папка пуста', outfit('Regular'), 13, VSCODE_MUTED, 400));
      ...
      const fileRow = al('HORIZONTAL', 'Welcome.txt row');
      fileRow.itemSpacing = 12;
      fileRow.counterAxisAlignItems = 'CENTER';
      fileRow.appendChild(txt('TXT', outfit('Bold'), 10, VSCODE_MUTED));
      fileRow.appendChild(txt('Welcome.txt', outfit('Regular'), 12, WHITE));
      fileRow.appendChild(txt('РУС', outfit('Regular'), 10, VSCODE_MUTED));
      fileRow.appendChild(txt('17:17', outfit('Regular'), 11, VSCODE_MUTED));
      fileRow.appendChild(txt('24.08.2026', outfit('Regular'), 11, VSCODE_MUTED));
      main.appendChild(fileRow);
    } else if (zipFile) {
      main.appendChild(txt('Welcome.txt', outfit('Regular'), 12, WHITE));
      const zipRow = al('HORIZONTAL', 'Welcome.zip row');
      zipRow.itemSpacing = 12;
      zipRow.counterAxisAlignItems = 'CENTER';
  ...
  function makeDesktopSim(name, overlay) {
    ...
    if (overlay === 'desknewfile') {
      const ic = al('VERTICAL', 'newfile.txt');
      ic.itemSpacing = 6;
      ic.primaryAxisAlignItems = 'CENTER';
      const box = figma.createRectangle();
      box.resize(36, 36);
      ...
      box.fills = [solid(WHITE)];
      ic.appendChild(box);
      const inp = al('HORIZONTAL', 'newfile rename input');
      inp.paddingLeft = inp.paddingRight = 6;
      inp.paddingTop = inp.paddingBottom = 2;
      inp.cornerRadius = 4;
      inp.fills = [solid({ r: 0.12, g: 0.13, b: 0.16 })];
      inp.strokes = [solid(BRAND)];
      inp.appendChild(txt('newfile.txt', outfit('Regular'), 11, WHITE));
      ic.appendChild(inp);
      icons.appendChild(ic);
    }
    ...
    if (!overlay) {
      ...
      deskWork.appendChild(menu);
    } else if (overlay === 'zipmenu') {
      deskWork.appendChild(deskWin('Проводник', '/Рабочий стол', '2 объектов', false, false, true));
      const menu = al('VERTICAL', 'ZIP extract menu');
      menu.itemSpacing = 0;
      menu.paddingTop = menu.paddingBottom = 6;
      menu.paddingLeft = menu.paddingRight = 6;
      menu.cornerRadius = 10;
  ...
  const deskMenuPaste = makeDesktopSim('Desktop paste menu /simulator?mode=desktop', 'menupaste');
  ...
  const deskZipExtract = makeDesktopSim('Desktop zip extract /simulator?mode=desktop', 'zipmenu');
  ...
  const typingCode = al('VERTICAL', 'Typing code');
  typingCode.itemSpacing = 12;
  typingCode.appendChild(typingHead('Тренировка'));
  const codeModeRow = al('HORIZONTAL', 'code modes');
  codeModeRow.itemSpacing = 8;
  for (const [label, on] of [
    ['Домашний ряд', false],
    ['Все буквы', false],
    ['Слова', false],
    ...
    chip.strokes = [solid(on ? BRAND : INK, on ? 1 : 0.12)];
    chip.appendChild(txt(label, outfit('SemiBold'), 13, on ? WHITE : INK));
    codeModeRow.appendChild(chip);
  }
  typingCode.appendChild(codeModeRow);
  const langRow = al('HORIZONTAL', 'code langs');
  langRow.itemSpacing = 4;
  for (const [label, on] of [
    ['python', false],
    ['javascript', true],
    ['html', false],
    ...
    chip.cornerRadius = 8;
    chip.fills = on ? [solid(BRAND)] : [TRANSPARENT];
    chip.appendChild(txt(label, outfit('SemiBold'), 12, on ? WHITE : MUTED));
    langRow.appendChild(chip);
  }
  typingCode.appendChild(langRow);
  typingCode.appendChild(typingLiveStats('—'));
  const codeBox = al('VERTICAL', 'code prompt');
  codeBox.itemSpacing = 8;
  codeBox.paddingTop = codeBox.paddingBottom = 20;
  codeBox.paddingLeft = codeBox.paddingRight = 20;
  codeBox.cornerRadius = 24;
  codeBox.fills = [solid(WHITE)];
  codeBox.appendChild(txt('function init(config) {', outfit('Regular'), 16, INK, 640));
  codeBox.appendChild(txt('  if (!config.isValid) return false;', outfit('Regular'), 16, INK, 640));
  codeBox.appendChild(txt('  return system.boot(config);', outfit('Regular'), 16, INK, 640));
  codeBox.appendChild(txt('}', outfit('Regular'), 16, INK, 640));
  typingCode.appendChild(codeBox);
  const codeCtl = al('HORIZONTAL', 'code typing controls');
  codeCtl.primaryAxisAlignItems = 'SPACE_BETWEEN';
  codeCtl.resize(680, 10);
  codeCtl.layoutSizingHorizontal = 'FIXED';
  codeCtl.layoutSizingVertical = 'HUG';
  codeCtl.appendChild(instSecondary('Ещё раз'));
  ...
  const trainHint = al('VERTICAL', 'Training hint');
  trainHint.itemSpacing = 12;
  trainHint.appendChild(txt('Тренировка горячих клавиш', outfit('SemiBold'), 24, INK));
  const hintMeta = al('HORIZONTAL', 'hint meta');
  hintMeta.primaryAxisAlignItems = 'SPACE_BETWEEN';
  hintMeta.counterAxisAlignItems = 'CENTER';
  hintMeta.resize(640, 10);
  hintMeta.layoutSizingHorizontal = 'FIXED';
  hintMeta.layoutSizingVertical = 'HUG';
  const hintProg = al('VERTICAL', 'hint progress');
  hintProg.itemSpacing = 6;
  hintProg.appendChild(txt('Задание 1 из 13', outfit('Regular'), 13, MUTED));
  hintProg.appendChild(instProgress('Value=Partial') || progressBar(220, 0.08, BRAND, 6));
  hintMeta.appendChild(hintProg);
  const hintStats = al('HORIZONTAL', 'hint stats');
  hintStats.itemSpacing = 16;
  hintStats.appendChild(txt('Серия: 0', outfit('SemiBold'), 13, MUTED));
  hintStats.appendChild(txt('✓ Верно: 0', outfit('SemiBold'), 13, MUTED));
  hintMeta.appendChild(hintStats);
  trainHint.appendChild(hintMeta);
  const hintCard = al('VERTICAL', 'hint trainer front');
  hintCard.itemSpacing = 16;
  hintCard.paddingTop = hintCard.paddingBottom = 24;
  hintCard.paddingLeft = hintCard.paddingRight = 24;
  hintCard.cornerRadius = 24;
  hintCard.fills = [solid(WHITE)];
  hintCard.strokes = [solid(BRAND, 0.18)];
  hintCard.primaryAxisAlignItems = 'CENTER';
  hintCard.resize(640, 10);
  hintCard.layoutSizingHorizontal = 'FIXED';
  hintCard.layoutSizingVertical = 'HUG';
  hintCard.appendChild(txt('Слово вправо', outfit('SemiBold'), 20, INK));
  hintCard.appendChild(txt('По словам', outfit('Regular'), 13, MUTED));
  const hintKeys = al('HORIZONTAL', 'hint KeyCombo');
  hintKeys.itemSpacing = 8;
  hintKeys.counterAxisAlignItems = 'CENTER';
  for (const k of ['Ctrl', '?']) {
    const cap = instKeyCap(k);
    if (cap) {
      hintKeys.appendChild(cap);
      continue;
    }
    const box = al('HORIZONTAL', k);
    box.paddingLeft = box.paddingRight = 12;
    ...
    box.strokes = [solid(INK)];
    box.appendChild(txt(k, outfit('SemiBold'), 14, INK));
    hintKeys.appendChild(box);
  }
  hintCard.appendChild(hintKeys);
  hintCard.appendChild(txt('Начните с Ctrl', outfit('Regular'), 13, MUTED));
  const hintBtns = al('HORIZONTAL', 'hint actions');
  hintBtns.itemSpacing = 8;
  hintBtns.appendChild(instSecondary('Подсказка'));
  hintBtns.appendChild(ghostBtn('Объяснение'));
  ...
  const trainHint3 = al('VERTICAL', 'Training hint 3');
  trainHint3.itemSpacing = 12;
  trainHint3.appendChild(txt('Тренировка горячих клавиш', outfit('SemiBold'), 24, INK));
  const hint3Meta = al('HORIZONTAL', 'hint3 meta');
  hint3Meta.primaryAxisAlignItems = 'SPACE_BETWEEN';
  hint3Meta.counterAxisAlignItems = 'CENTER';
  hint3Meta.resize(640, 10);
  hint3Meta.layoutSizingHorizontal = 'FIXED';
  hint3Meta.layoutSizingVertical = 'HUG';
  const hint3Prog = al('VERTICAL', 'hint3 progress');
  hint3Prog.itemSpacing = 6;
  hint3Prog.appendChild(txt('Задание 1 из 13', outfit('Regular'), 13, MUTED));
  hint3Prog.appendChild(instProgress('Value=Partial') || progressBar(220, 0.08, BRAND, 6));
  hint3Meta.appendChild(hint3Prog);
  const hint3Stats = al('HORIZONTAL', 'hint3 stats');
  hint3Stats.itemSpacing = 16;
  hint3Stats.appendChild(txt('Серия: 0', outfit('SemiBold'), 13, MUTED));
  hint3Stats.appendChild(txt('✓ Верно: 0', outfit('SemiBold'), 13, MUTED));
  hint3Meta.appendChild(hint3Stats);
  trainHint3.appendChild(hint3Meta);
  const hint3Card = al('VERTICAL', 'hint3 trainer front');
  hint3Card.itemSpacing = 16;
  hint3Card.paddingTop = hint3Card.paddingBottom = 24;
  hint3Card.paddingLeft = hint3Card.paddingRight = 24;
  hint3Card.cornerRadius = 24;
  hint3Card.fills = [solid(WHITE)];
  hint3Card.strokes = [solid(BRAND, 0.18)];
  hint3Card.primaryAxisAlignItems = 'CENTER';
  hint3Card.resize(640, 10);
  hint3Card.layoutSizingHorizontal = 'FIXED';
  hint3Card.layoutSizingVertical = 'HUG';
  hint3Card.appendChild(txt('В конец строки', outfit('SemiBold'), 20, INK));
  const hint3Keys = al('HORIZONTAL', 'hint3 KeyCombo');
  hint3Keys.itemSpacing = 8;
  hint3Keys.counterAxisAlignItems = 'CENTER';
  for (const k of ['End']) {
    const cap = instKeyCap(k);
    if (cap) {
      hint3Keys.appendChild(cap);
      continue;
    }
    const box = al('HORIZONTAL', k);
    box.paddingLeft = box.paddingRight = 12;
    ...
    box.strokes = [solid(INK)];
    box.appendChild(txt(k, outfit('SemiBold'), 14, INK));
    hint3Keys.appendChild(box);
  }
  hint3Card.appendChild(hint3Keys);
  hint3Card.appendChild(txt('Нажмите: End', outfit('Regular'), 13, MUTED));
  const hint3Btns = al('HORIZONTAL', 'hint3 actions');
  hint3Btns.itemSpacing = 8;
  hint3Btns.appendChild(ghostBtn('Объяснение'));
  hint3Btns.appendChild(ghostBtn('Пропустить'));
  hint3Card.appendChild(hint3Btns);
  trainHint3.appendChild(hint3Card);

  const trainRetry = al('VERTICAL', 'Training retry');
  trainRetry.itemSpacing = 12;
  trainRetry.appendChild(txt('Тренировка горячих клавиш', outfit('SemiBold'), 24, INK));
  const retryMeta = al('HORIZONTAL', 'retry meta');
  retryMeta.primaryAxisAlignItems = 'SPACE_BETWEEN';
  retryMeta.counterAxisAlignItems = 'CENTER';
  retryMeta.resize(640, 10);
  retryMeta.layoutSizingHorizontal = 'FIXED';
  retryMeta.layoutSizingVertical = 'HUG';
  const retryProg = al('VERTICAL', 'retry progress');
  retryProg.itemSpacing = 6;
  const retryTask = al('HORIZONTAL', 'retry task');
  retryTask.itemSpacing = 8;
  retryTask.counterAxisAlignItems = 'CENTER';
  retryTask.appendChild(txt('Задание 2 из 2', outfit('Regular'), 13, MUTED));
  retryTask.appendChild(txt('повтор', outfit('Medium'), 12, BRAND800));
  retryProg.appendChild(retryTask);
  retryProg.appendChild(instProgress('Value=Partial') || progressBar(220, 0.5, BRAND, 6));
  retryMeta.appendChild(retryProg);
  const retryStats = al('HORIZONTAL', 'retry stats');
  retryStats.itemSpacing = 16;
  retryStats.appendChild(txt('Серия: 1', outfit('SemiBold'), 13, MUTED));
  retryStats.appendChild(txt('✓ Верно: 1', outfit('SemiBold'), 13, MUTED));
  retryMeta.appendChild(retryStats);
  trainRetry.appendChild(retryMeta);
  const retryCard = al('VERTICAL', 'retry trainer front');
  retryCard.itemSpacing = 16;
  retryCard.paddingTop = retryCard.paddingBottom = 24;
  retryCard.paddingLeft = retryCard.paddingRight = 24;
  retryCard.cornerRadius = 24;
  retryCard.fills = [solid(WHITE)];
  retryCard.strokes = [solid(BRAND, 0.18)];
  retryCard.primaryAxisAlignItems = 'CENTER';
  retryCard.resize(640, 10);
  retryCard.layoutSizingHorizontal = 'FIXED';
  retryCard.layoutSizingVertical = 'HUG';
  retryCard.appendChild(txt('В конец строки', outfit('SemiBold'), 20, INK));
  const retryKeys = al('HORIZONTAL', 'retry KeyCombo');
  retryKeys.itemSpacing = 8;
  retryKeys.counterAxisAlignItems = 'CENTER';
  for (const k of ['End']) {
    const cap = instKeyCap(k);
    if (cap) {
      retryKeys.appendChild(cap);
      continue;
    }
    const box = al('HORIZONTAL', k);
    box.paddingLeft = box.paddingRight = 12;
    ...
    box.strokes = [solid(INK)];
    box.appendChild(txt(k, outfit('SemiBold'), 14, INK));
    retryKeys.appendChild(box);
  }
  retryCard.appendChild(retryKeys);
  retryCard.appendChild(txt('Нажмите сочетание на клавиатуре', outfit('Regular'), 13, MUTED));
  const retryBtns = al('HORIZONTAL', 'retry actions');
  retryBtns.itemSpacing = 8;
  retryBtns.appendChild(instSecondary('Подсказка'));
  retryBtns.appendChild(ghostBtn('Объяснение'));
  retryBtns.appendChild(ghostBtn('Пропустить'));
  retryCard.appendChild(retryBtns);
  trainRetry.appendChild(retryCard);
  ...
  const examCert = al('VERTICAL', 'Exam certificate');
  examCert.itemSpacing = 12;
  examCert.paddingTop = examCert.paddingBottom = 24;
  examCert.paddingLeft = examCert.paddingRight = 24;
  examCert.cornerRadius = 24;
  examCert.fills = [solid(WHITE)];
  examCert.resize(520, 10);
  examCert.layoutSizingHorizontal = 'FIXED';
  examCert.layoutSizingVertical = 'HUG';
  examCert.appendChild(txt('ИТОГИ ЭКЗАМЕНА', outfit('Bold'), 11, BRAND800));
  examCert.appendChild(txt('Сессия завершена', fraunces('Bold'), 30, INK));
  const certGradeRow = al('HORIZONTAL', 'cert grade');
  certGradeRow.itemSpacing = 16;
  certGradeRow.counterAxisAlignItems = 'CENTER';
  const certGradeBox = al('HORIZONTAL', 'A');
  certGradeBox.primaryAxisAlignItems = 'CENTER';
  certGradeBox.counterAxisAlignItems = 'CENTER';
  certGradeBox.resize(64, 64);
  certGradeBox.layoutSizingHorizontal = 'FIXED';
  certGradeBox.layoutSizingVertical = 'FIXED';
  certGradeBox.cornerRadius = 16;
  certGradeBox.strokes = [solid(SUCCESS)];
  certGradeBox.appendChild(txt('A', outfit('Bold'), 24, SUCCESS));
  certGradeRow.appendChild(certGradeBox);
  const certGradeCopy = al('VERTICAL', 'cert pct');
  certGradeCopy.itemSpacing = 4;
  certGradeCopy.appendChild(txt('100%', outfit('Bold'), 30, INK));
  certGradeCopy.appendChild(txt('Отличный результат', outfit('Regular'), 13, MUTED));
  certGradeRow.appendChild(certGradeCopy);
  examCert.appendChild(certGradeRow);
  examCert.appendChild(progressBar(472, 1, SUCCESS, 8));
  const certStats = al('HORIZONTAL', 'cert result stats');
  certStats.itemSpacing = 8;
  certStats.layoutWrap = 'WRAP';
  certStats.resize(472, 10);
  certStats.layoutSizingHorizontal = 'FIXED';
  certStats.layoutSizingVertical = 'HUG';
  for (const [k, v] of [
    ['Верно', '1'],
    ['Ошибок', '0'],
    ...
    st.appendChild(txt(k, outfit('Bold'), 10, MUTED));
    st.appendChild(txt(v, outfit('Bold'), 20, INK));
    certStats.appendChild(st);
  }
  examCert.appendChild(certStats);
  const certCard = al('VERTICAL', 'CertificateCard');
  certCard.itemSpacing = 8;
  certCard.primaryAxisAlignItems = 'CENTER';
  certCard.paddingTop = certCard.paddingBottom = 24;
  certCard.paddingLeft = certCard.paddingRight = 24;
  certCard.cornerRadius = 16;
  certCard.fills = [solid(BRAND50)];
  certCard.strokes = [solid(BRAND, 0.3)];
  certCard.resize(472, 10);
  certCard.layoutSizingHorizontal = 'FIXED';
  certCard.layoutSizingVertical = 'HUG';
  certCard.appendChild(txt('СЕРТИФИКАТ', outfit('Bold'), 11, BRAND800));
  certCard.appendChild(txt('KeyMaster', fraunces('Bold'), 18, INK));
  certCard.appendChild(txt('Успешно сдан экзамен — 100%', outfit('Regular'), 13, MUTED));
  examCert.appendChild(certCard);
  const certBtns = al('HORIZONTAL', 'cert actions');
  certBtns.itemSpacing = 8;
  certBtns.appendChild(instPrimary('Новый экзамен'));
  certBtns.appendChild(secondaryBtn('К пути'));
  examCert.appendChild(certBtns);
  ...
  board.appendChild(
    section('MarketingShell', [
      ...
      marketingPage('Lesson task guest /lessons/:id', 'Курсы', true, [lessonTaskGuest]),
      ...
      marketingPage('Admin edit lesson /admin', '', false, [
        ...adminHeader('Курсы'),
        (() => {
          const back = al('HORIZONTAL', 'back edit lesson');
          back.itemSpacing = 8;
          back.counterAxisAlignItems = 'CENTER';
          back.appendChild(txt('←', outfit('SemiBold'), 13, BRAND800));
          back.appendChild(txt('Назад к курсам', outfit('SemiBold'), 13, BRAND800));
        ...
        txt('Категории', outfit('SemiBold'), 18, INK),
        (() => {
          const cat = al('VERTICAL', 'Edit lesson category');
          cat.itemSpacing = 10;
          cat.paddingTop = cat.paddingBottom = 16;
          cat.paddingLeft = cat.paddingRight = 16;
          cat.cornerRadius = 16;
          ...
          cat.layoutSizingHorizontal = 'FIXED';
          cat.layoutSizingVertical = 'HUG';
          const head = al('HORIZONTAL', 'edit lesson cat head');
          head.primaryAxisAlignItems = 'SPACE_BETWEEN';
          head.counterAxisAlignItems = 'CENTER';
          const copy = al('VERTICAL', 'edit lesson cat copy');
          copy.itemSpacing = 4;
          copy.appendChild(txt('Файлы и папки', outfit('SemiBold'), 15, INK));
          copy.appendChild(txt('basics · 3 Уроки', outfit('Regular'), 12, MUTED));
          head.appendChild(copy);
          ...
          cat.appendChild(head);
          head.layoutSizingHorizontal = 'FILL';
          const form = al('VERTICAL', 'Edit lesson form');
          form.itemSpacing = 8;
          form.paddingTop = form.paddingBottom = 12;
          form.paddingLeft = form.paddingRight = 12;
          form.cornerRadius = 12;
          form.strokes = [solid(INK, 0.08)];
          const grid = al('HORIZONTAL', 'edit lesson grid');
          grid.itemSpacing = 12;
          grid.layoutWrap = 'WRAP';
          grid.resize(800, 10);
          grid.layoutSizingHorizontal = 'FIXED';
          grid.layoutSizingVertical = 'HUG';
          grid.appendChild(field('Название', 'Файл и папка', INK));
          grid.appendChild(field('Клавиши', '', INK));
          grid.appendChild(field('Задание', 'Поймите разницу между файлом и папкой', INK));
          grid.appendChild(field('XP за урок', '15', INK));
          form.appendChild(grid);
          form.appendChild(
            field(
              'Описание',
          ...
          form.appendChild(
            field('Зачем нужно', 'Папка — «конверт»; файл — одна заметка, фото или программа', INK),
          );
          const actions = al('HORIZONTAL', 'edit lesson actions');
          actions.itemSpacing = 8;
          actions.appendChild(instPrimary('Сохранить'));
          actions.appendChild(instSecondary('Отмена'));
          form.appendChild(actions);
          cat.appendChild(form);
          const nextLesson = al('VERTICAL', 'Расширение файла edit-lesson');
          nextLesson.itemSpacing = 2;
          nextLesson.paddingTop = nextLesson.paddingBottom = 8;
          nextLesson.paddingLeft = nextLesson.paddingRight = 12;
          nextLesson.cornerRadius = 8;
          nextLesson.strokes = [solid(INK, 0.08)];
          nextLesson.appendChild(txt('Расширение файла', outfit('Medium'), 14, INK));
          nextLesson.appendChild(txt('· +15 XP', outfit('Regular'), 11, BRAND800));
          cat.appendChild(nextLesson);
          cat.appendChild(
            txt(
              'Инлайн-редактирование урока (не пустая форма Урок). Не дублировать остальные 15 уроков.',
              outfit('Regular'),
              12,
              MUTED,
              800,
  ...
  board.appendChild(
    section('PracticeShell', [
      ...
      practicePage('Training hint 3 /training', 'Hotkeys', [trainHint3]),
      practicePage('Training retry /training', 'Hotkeys', [trainRetry]),
      ...
      practicePage('Training done weak /training', 'Hotkeys', [trainDoneWeak]),
      ...
      practicePage('Quiz finished /quiz', 'Основы hotkeys', [quizFinished]),
      practicePage('Exam setup /exam', 'Экзамен', [exam]),
      practicePage('Exam capped /exam', 'Экзамен', [examCapped]),
      ...
      practicePage('Exam certificate /exam', 'Экзамен', [examCert]),
