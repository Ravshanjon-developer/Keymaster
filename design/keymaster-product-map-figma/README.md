...
| Page | Contents |
|---|---|
| `01 — Product Map` | Unique as-is recreations. Includes leaderboard empty/period-empty/API-down/loading, achievements empty, lesson loading, study-only authed/learned, task/desktop-task done, training empty/done, review empty, course-detail loading, six `/courses/:slug` layouts, desktop-task lesson (`Папка Practice`), path spine, keyboard gate, authed catalog `0/N`, mobile computer-basics (Desktop CTA), authed mobile Home/Leaderboard/catalog, mobile Register, and mobile Dashboard. |
| `02 — User Flows` | F1–F12 from the live router + capture strips |
| `03 — Existing Screens` | As-is desktop, dark, and state captures (88), including leaderboard empty/period-empty/API-down/loading, achievements empty, lesson loading/study-only authed/learned/task done, training empty, review empty, course-detail loading, path full-scroll, authed catalog, six `/courses/:slug` layouts, authed leaderboard, catalog start filter, and desktop-task lesson (`Папка Practice`) |
...
| `07 — Mobile Screens` | 390 captures covering all four BottomNav tabs plus Register (BottomNav hidden), Dashboard (Кабинет, no tab), and authed computer-basics course detail (Desktop CTA). Guest vs authed Home, Leaderboard, and Courses. Authed catalog shows `0/16 сочетаний`. `mobile-authed-08-training-gate.jpg` is PracticeKeyboardGate on `/exam`. Closed authed navbar shows Выйти because `.btn-secondary { inline-flex }` wins over `hidden`. |
