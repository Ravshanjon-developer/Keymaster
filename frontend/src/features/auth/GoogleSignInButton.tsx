import { Loader2 } from 'lucide-react'
import { useState } from 'react'
import toast from 'react-hot-toast'

import { useAuthStore } from '@/features/auth/authStore'
import { isSupabaseAuth } from '@/shared/lib/supabase'
import { useT } from '@/shared/i18n'
import { ApiError } from '@/shared/lib/api'
import { cn } from '@/shared/lib/utils'

function GoogleIcon() {
  return (
    <svg className="h-5 w-5 shrink-0" viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  )
}

export function GoogleSignInBlock({ className }: { className?: string }) {
  const t = useT()
  const loginWithGoogle = useAuthStore((s) => s.loginWithGoogle)
  const [loading, setLoading] = useState(false)

  if (!isSupabaseAuth) return null

  async function onGoogle() {
    setLoading(true)
    try {
      await loginWithGoogle()
    } catch (err) {
      const msg = err instanceof ApiError ? err.message : t('auth.googleSignInFail')
      toast.error(msg)
      setLoading(false)
    }
  }

  return (
    <div className={cn('space-y-5', className)}>
      <button
        type="button"
        disabled={loading}
        aria-busy={loading}
        onClick={() => void onGoogle()}
        className={cn(
          'flex min-h-[2.75rem] w-full items-center justify-center gap-3 rounded-xl border px-4 text-sm font-semibold shadow-sm transition',
          'border-[var(--border-default)] bg-white text-[var(--text-primary)]',
          'hover:bg-slate-50 active:scale-[0.99]',
          'focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--focus-ring)]',
          'disabled:cursor-not-allowed disabled:opacity-70',
          'dark:border-slate-600 dark:bg-slate-900 dark:hover:bg-slate-800',
        )}
      >
        {loading ? (
          <Loader2 className="h-5 w-5 shrink-0 animate-spin text-[var(--text-muted)]" aria-hidden />
        ) : (
          <GoogleIcon />
        )}
        <span>{loading ? t('auth.googleRedirecting') : t('auth.continueWithGoogle')}</span>
      </button>
      <div className="relative" role="separator" aria-label={t('auth.orDivider')}>
        <div className="border-t border-[var(--border-default)]" />
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--bg-elevated)] px-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)]">
          {t('auth.orDivider')}
        </span>
      </div>
    </div>
  )
}